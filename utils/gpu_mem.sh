if [ -z "${MLPERF_UTILS_GPUMEM_SH_INCLUDED:-}" ]; then
    ...
    function mlc_utils_srun_disp_gpu_mem {
        if [ "${ENABLE_DISP_GPU_MEM:-0}" -eq 1 ]; then
            echo "[gpu_mem.sh] GPU memory usage: start"
            echo "{"
            echo "  \"time\": \"$(date)\","
            echo "  \"gpu_memory\": \""
            srun --overlap -l --ntasks=$SLURM_JOB_NUM_NODES --ntasks-per-node 1 $(dirname "${BASH_SOURCE[0]}")/disp_gpu_mem.sh
            echo "\""
            echo "}"
            echo "[gpu_mem.sh] GPU memory usage: end"
        fi
    }
    ...
    function mlc_utils_sbatch_disp_gpu_mem() {
        if [ "${ENABLE_DISP_GPU_MEM:-0}" -eq 1 ]; then
            sbatch -p $SLURM_JOB_PARTITION ${SLURM_JOB_RESERVATION:+--reservation ${SLURM_JOB_RESERVATION}} -w $SLURM_NODELIST -t2 \
                ...
                --job-name "disp_gpu_mem-post-$SLURM_JOB_NAME" \
                --wrap="
                echo \"[gpu_mem.sh] GPU memory usage: start\"
                echo \"{\"
                echo \"  \\\"time\\\": \\\"\$(date)\\\",\"
                echo \"  \\\"gpu_memory\\\": \\\"\"
                srun -l $(dirname "${BASH_SOURCE[0]}")/disp_gpu_mem.sh
                echo \"\\\"\"
                echo \"}\"
                echo \"[gpu_mem.sh] GPU memory usage: end\""
        fi
