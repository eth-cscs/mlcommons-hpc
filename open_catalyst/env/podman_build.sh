...
if nvidia-smi &> /dev/null; then
    ...
    podman_build_enroot_import ${BASE_CONTAINER_REGISTRY}-open_catalyst:${BASE_TAG_SHORT} -f Dockerfile --build-arg BASE_IMAGE=${BASE_IMAGE} .
    set +x
    ...
    set -x
    podman_build_enroot_import ${BASE_CONTAINER_REGISTRY}-open_catalyst:${BASE_TAG_SHORT} -f Dockerfile-rocm --build-arg BASE_IMAGE=${BASE_IMAGE} .
    set +x
else
